    ...
    it("should produce non-empty set strategy payload") {
      ...
      (set personalizationStrategy strategy).build() should be(
        HttpPayload(
          POST,
          Seq("1", "recommendation", "personalization", "strategy"),
          body = Some("""{"eventsScoring":{"event 1":{"score":10,"type":"click"},"event 2":{"score":20,"type":"conversion"}},"facetsScoring":{"event 3":{"score":30}}}"""),
          isSearch = false,
          requestOptions = None,
        )
      )
